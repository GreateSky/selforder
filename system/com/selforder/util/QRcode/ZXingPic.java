...
public class ZXingPic {
	...
	public BufferedImage fileToBufferedImage(BitMatrix bm) {
		...
		try {
			...
			for (int x = 0; x < w; x++) {
				for (int y = 0; y < h; y++) {
					image.setRGB(x, y, bm.get(x, y) ? 0xFF000000 : 0xFFFFFFFF);//0xFFCCDDEE
				}
			}
	...
	public void getQR_CODEBufferedImage2(String content, int width, int height,
			Map<EncodeHintType, ?> hints,OutputStream ops) {
		MultiFormatWriter multiFormatWriter = null;
		BitMatrix bm = null;
		BufferedImage image = null;
		try {
			...
			// 参数顺序分别为：编码内容，编码类型，生成图片宽度，生成图片高度，设置参数
			bm = multiFormatWriter.encode(content, BarcodeFormat.QR_CODE, width,
					height, hints);
			decodeQR_CODE2OutputStream(bm,"png",ops);
		} catch (WriterException e) {
			e.printStackTrace();
		}
	}
